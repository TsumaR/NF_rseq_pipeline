...
def main():
    ...
    ap.add_argument('-i', '--input', dest='fin', action='store',
                    metavar='FILE', default=INFILE,
                    help='input file name, default=%s' % INFILE)
    ...
    data = []
    for s in smpl.sequence_id:
        print(s, file=sys.stderr)
        spath = '%s/%s/%s' % (args.root, s, args.fin)
    ...
    with Extend.IO.sopen(args.fout) as fo:
        ...
        header.extend(smpl.sequence_id)
